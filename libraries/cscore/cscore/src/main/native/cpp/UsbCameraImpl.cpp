...
void UsbCameraImpl::DeviceConnect() {
  ...
  SINFO("Connect to USB camera on " << m_path);
...
CS_StatusValue UsbCameraImpl::DeviceProcessCommand(
  ...
  if (msg.kind == Message::kCmdSetMode ||
    ...
    return CS_OK;
  } else if (msg.kind == Message::kAbort) {
    m_active = false;
    return CS_OK;
  }else {
    return CS_OK;
...
void UsbCameraImpl::DisconnectCamera() {
  DeviceStreamOff();
  DeviceDisconnect();
  m_active = false;
  m_responseCv.notify_all();
  Send(Message{Message::kNone});
  Send(Message{Message::kAbort});
  if (m_cameraThread.joinable()) m_cameraThread.join();
  int fd = m_command_fd.exchange(-1);
  if (fd >= 0) close(fd);
  std::cout << "Disconnected USB Camera" << std::endl;
}
